...
export default class Picker extends Component {
  ...
  render() {
    ...
    let reformatPickerData = [];
    if(pickerData.length==29){
     reformatPickerData = pickerData.filter(item => item.value !=29)
    } else {
        reformatPickerData = pickerData;
    }
    return (
      <WheelCurvedPicker
        {...props}
      ...
      >
        {reformatPickerData.map((data, index) => (
          <PickerItem
            key={index}
